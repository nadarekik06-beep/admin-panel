...
/**
 * FILE: app/admin/complaints/page.tsx  (Admin Panel — port 3001)  ← REPLACE
 *
 * Changes from v1:
 *   - Status config includes seller_rejected_pending_admin
 *   - Drawer shows 2 new action buttons when status = seller_rejected_pending_admin:
 *       "Confirm Rejection" → confirmRejection()
 *       "Override → Approve" → overrideToApproved()
 *   - Stats row includes "Needs Admin" badge with orange alert
 *   - Table highlights seller_rejected rows in orange
 */
...
function StatCard({ label, value, color, icon, alert }: {
  ...
  return (
    <div style={{ background: '#fff', borderRadius: 16,
      border: alert && value > 0 ? `2px solid ${ORANGE}` : '1.5px solid #f1f5f9',
  ...
  return (
    <>
      ...
      <div style={{ position: 'fixed', top: '50%', left: '50%', transform: 'translate(-50%,-50%)',
        width: '100%', maxWidth: 480, background: '#fff', borderRadius: 20, padding: 28,
        boxShadow: '0 32px 80px rgba(0,0,0,0.2)', zIndex: 10001 }}>
        <h3 style={{ fontSize: 16, fontWeight: 900, color: '#0f172a', margin: '0 0 8px' }}>Reject Complaint</h3>
...
function ComplaintDrawer({ complaint, onClose, onRefresh }: {
  ...
  return (
    <>
      ...
      <div style={{ position: 'fixed', top: 0, right: 0, bottom: 0, width: '100%', maxWidth: 580,
        background: '#fff', boxShadow: '-12px 0 48px rgba(0,0,0,0.15)',
        ...
        <div style={{ padding: '20px 24px 16px', borderBottom: '1px solid #f1f5f9',
          ...
          <button onClick={onClose}
            style={{ width: 34, height: 34, borderRadius: '50%', border: '1.5px solid #e5e7eb',
        ...
        <div style={{ flex: 1, padding: '20px 24px', display: 'flex', flexDirection: 'column', gap: 20 }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
            <div style={{ background: '#f8fafc', borderRadius: 12, padding: '14px 16px' }}>
              <p style={{ fontSize: 10, fontWeight: 800, color: '#94a3b8', textTransform: 'uppercase', letterSpacing: '0.07em', margin: '0 0 6px' }}>Client</p>
              <p style={{ fontSize: 14, fontWeight: 800, color: '#0f172a', margin: '0 0 2px' }}>{complaint.user?.name ?? '—'}</p>
              <p style={{ fontSize: 12, color: '#64748b', margin: 0 }}>{complaint.user?.email ?? '—'}</p>
            </div>
            <div style={{ background: '#f8fafc', borderRadius: 12, padding: '14px 16px' }}>
              <p style={{ fontSize: 10, fontWeight: 800, color: '#94a3b8', textTransform: 'uppercase', letterSpacing: '0.07em', margin: '0 0 6px' }}>Seller</p>
              <p style={{ fontSize: 14, fontWeight: 800, color: '#0f172a', margin: '0 0 2px' }}>{complaint.seller?.name ?? '—'}</p>
              <p style={{ fontSize: 12, color: '#64748b', margin: 0 }}>{complaint.seller?.email ?? '—'}</p>
            </div>
          </div>

          {/* Order */}
          <div style={{ background: '#f8fafc', borderRadius: 12, padding: '14px 16px' }}>
            <p style={{ fontSize: 10, fontWeight: 800, color: '#94a3b8', textTransform: 'uppercase', letterSpacing: '0.07em', margin: '0 0 8px' }}>Order</p>
            <p style={{ fontSize: 14, fontWeight: 800, color: '#0f172a', margin: '0 0 4px', fontFamily: 'monospace' }}>
          ...
          {complaint.image_url && (
            <div>
              <p style={{ fontSize: 10, fontWeight: 800, color: '#94a3b8', textTransform: 'uppercase', letterSpacing: '0.07em', margin: '0 0 8px' }}>Proof Photo</p>
              <a href={complaint.image_url} target="_blank" rel="noreferrer">
                <img src={complaint.image_url} alt="Proof"
                  style={{ width: '100%', maxHeight: 260, objectFit: 'contain', borderRadius: 12,
                    border: '1.5px solid #e5e7eb', background: '#f8fafc', cursor: 'zoom-in' }} />
              </a>
            </div>
          )}
          ...
          {complaint.seller_note && (
            <div>
              <p style={{ fontSize: 10, fontWeight: 800, color: '#94a3b8', textTransform: 'uppercase', letterSpacing: '0.07em', margin: '0 0 8px' }}>
                Seller Response
              ...
              <div style={{ background: 'rgba(30,64,175,0.06)', border: '1.5px solid rgba(30,64,175,0.2)', borderRadius: 12, padding: '14px 16px' }}>
                <p style={{ fontSize: 13, color: '#374151', margin: 0, lineHeight: 1.7 }}>{complaint.seller_note}</p>
                {complaint.rejection_reason && (
                  <div style={{ marginTop: 10, paddingTop: 10, borderTop: '1px solid rgba(30,64,175,0.15)' }}>
                    ...
                    <p style={{ fontSize: 13, color: '#374151', margin: 0 }}>{complaint.rejection_reason}</p>
                  </div>
                )}
              </div>
            </div>
          ...
          {/* SELLER_REJECTED banner — admin must act */}
          {isSellerRejected && (
            <div style={{ background: 'rgba(249,115,22,0.08)', border: `2px solid rgba(249,115,22,0.4)`,
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
                <button onClick={() => act(() => adminComplaintApi.overrideToApproved(complaint.id),
                  ...
                  disabled={acting}
                  style={{ padding: '12px', background: GREEN, color: '#fff', border: 'none',
                    borderRadius: 10, fontSize: 13, fontWeight: 800, cursor: acting ? 'not-allowed' : 'pointer',
                    fontFamily: 'inherit' }}>
                  ✅ Override → Approve
                </button>
                <button onClick={() => act(() => adminComplaintApi.confirmRejection(complaint.id),
                  ...
                  disabled={acting}
                  style={{ padding: '12px', background: '#ef4444', color: '#fff', border: 'none',
                    borderRadius: 10, fontSize: 13, fontWeight: 800, cursor: acting ? 'not-allowed' : 'pointer',
                    fontFamily: 'inherit' }}>
                  ❌ Confirm Rejection
                </button>
          ...
          {isResolved && (
            <div style={{
              ...
              <p style={{ fontSize: 12, fontWeight: 800, margin: '0 0 6px', textTransform: 'uppercase',
        ...
        {/* Standard approve/reject footer (for non-seller-rejected, non-resolved) */}
        {!isResolved && !isSellerRejected && (
          <div style={{ padding: '16px 24px', borderTop: '1px solid #f1f5f9',
            ...
            <button onClick={() => setRejectModal(true)} disabled={acting}
              style={{ flex: 1, padding: 12, background: 'rgba(239,68,68,0.08)', color: '#ef4444',
            ...
            <button onClick={() => act(() => adminComplaintApi.approve(complaint.id), '✅ Complaint approved.')}
              disabled={acting}
              style={{ flex: 1, padding: 12, background: acting ? '#94a3b8' : GREEN,
...
function ComplaintTableRow({ complaint, onSelect }: { complaint: Complaint; onSelect: (c: Complaint) => void }) {
  ...
  return (
    <tr onClick={() => onSelect(complaint)} style={{ cursor: 'pointer', transition: 'background 0.15s ease',
      background: needsAction ? 'rgba(249,115,22,0.04)' : 'transparent' }}
      onMouseEnter={e => (e.currentTarget.style.background = needsAction ? 'rgba(249,115,22,0.08)' : '#fafafa')}
      ...
      <td style={{ padding: '13px 16px', borderBottom: '1px solid #f1f5f9' }}>
        <span style={{ fontFamily: 'monospace', fontWeight: 700, fontSize: 12, color: '#0f172a' }}>#{complaint.id}</span>
        {needsAction && <span style={{ marginLeft: 6, fontSize: 14 }}>⚠️</span>}
      </td>
      <td style={{ padding: '13px 16px', borderBottom: '1px solid #f1f5f9' }}>
        <div style={{ fontSize: 13, fontWeight: 700, color: '#0f172a' }}>{complaint.user?.name ?? '—'}</div>
        <div style={{ fontSize: 11, color: '#94a3b8' }}>{complaint.user?.email ?? ''}</div>
      </td>
      <td style={{ padding: '13px 16px', borderBottom: '1px solid #f1f5f9', fontSize: 13, color: '#374151' }}>
        {COMPLAINT_TYPE_LABELS[complaint.complaint_type]}
      </td>
      <td style={{ padding: '13px 16px', borderBottom: '1px solid #f1f5f9' }}>
      ...
      <td style={{ padding: '13px 16px', borderBottom: '1px solid #f1f5f9', fontSize: 11, color: '#94a3b8', fontWeight: 600 }}>
        {new Date(complaint.created_at).toLocaleDateString('en-GB', { day: 'numeric', month: 'short', year: 'numeric' })}
      </td>
      <td style={{ padding: '13px 16px', borderBottom: '1px solid #f1f5f9', textAlign: 'right' }}>
        <button onClick={e => { e.stopPropagation(); onSelect(complaint) }}
          style={{ padding: '6px 14px', background: '#f8fafc', color: '#374151',
            border: '1.5px solid #e5e7eb', borderRadius: 8, fontSize: 12, fontWeight: 700,
            cursor: 'pointer', fontFamily: 'inherit' }}>
          View →
        </button>
      </td>
...
export default function AdminComplaintsPage() {
  ...
  const statItems = stats ? [
    { label: 'Total',          value: stats.total,           color: '#64748b', icon: '📋', alert: false },
    ...
    <>
      ...
      <div style={{ fontFamily: "'DM Sans', sans-serif", padding: '28px 32px',
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 14, marginBottom: 28 }}>
          <div style={{ width: 48, height: 48, borderRadius: 14, background: 'rgba(220,38,38,0.08)',
            display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 22 }}>
            🚨
          </div>
          <div>
            <h1 style={{ fontSize: 24, fontWeight: 900, color: '#0f172a', margin: 0 }}>Complaint Management</h1>
        ...
        <div style={{ background: '#fff', borderRadius: 16, border: '1.5px solid #f1f5f9',
          ...
          <div style={{ flex: '1 1 200px' }}>
            <label style={{ fontSize: 11, fontWeight: 700, color: '#94a3b8', textTransform: 'uppercase',
              letterSpacing: '0.06em', display: 'block', marginBottom: 6 }}>Search</label>
            <input type="text" placeholder="Customer name, email, order #…" value={filterSearch}
              onChange={e => setFilterSearch(e.target.value)}
              style={{ width: '100%', padding: '9px 14px', borderRadius: 9, border: '1.5px solid #e5e7eb',
                fontSize: 13, outline: 'none', fontFamily: 'inherit', boxSizing: 'border-box' }} />
          </div>
          <div style={{ flex: '0 0 180px' }}>
            <label style={{ fontSize: 11, fontWeight: 700, color: '#94a3b8', textTransform: 'uppercase',
              letterSpacing: '0.06em', display: 'block', marginBottom: 6 }}>Status</label>
            <select value={filterStatus} onChange={e => setFilterStatus(e.target.value)}
              style={{ width: '100%', padding: '9px 14px', borderRadius: 9, border: '1.5px solid #e5e7eb',
                fontSize: 13, outline: 'none', fontFamily: 'inherit', background: '#fff', cursor: 'pointer' }}>
          ...
          <div style={{ flex: '0 0 150px' }}>
            <label style={{ fontSize: 11, fontWeight: 700, color: '#94a3b8', textTransform: 'uppercase',
              letterSpacing: '0.06em', display: 'block', marginBottom: 6 }}>From</label>
          ...
          <div style={{ flex: '0 0 150px' }}>
            <label style={{ fontSize: 11, fontWeight: 700, color: '#94a3b8', textTransform: 'uppercase',
              letterSpacing: '0.06em', display: 'block', marginBottom: 6 }}>To</label>
            <input type="date" value={filterToDate} onChange={e => setFilterToDate(e.target.value)}
              style={{ width: '100%', padding: '9px 14px', borderRadius: 9, border: '1.5px solid #e5e7eb',
                fontSize: 13, outline: 'none', fontFamily: 'inherit', boxSizing: 'border-box' }} />
          </div>
          <button onClick={() => { setFilterStatus(''); setFilterFromDate(''); setFilterToDate(''); setFilterSearch('') }}
            style={{ padding: '9px 18px', border: '1.5px solid #e5e7eb', borderRadius: 9,
        ...
        <div style={{ background: '#fff', borderRadius: 16, border: '1.5px solid #f1f5f9', overflow: 'hidden' }}>
          {loading ? (
            <div style={{ textAlign: 'center', padding: '60px 0' }}>
              <div style={{ width: 28, height: 28, border: '3px solid #f1f5f9', borderTopColor: RED,
                borderRadius: '50%', animation: 'spin 0.8s linear infinite', margin: '0 auto 12px' }} />
              <p style={{ color: '#94a3b8', fontSize: 13, fontWeight: 600 }}>Loading complaints…</p>
            </div>
          ) : complaints.length === 0 ? (
            <div style={{ textAlign: 'center', padding: '60px 0', color: '#94a3b8' }}>
              <div style={{ fontSize: 40, marginBottom: 12 }}>🚨</div>
              <p style={{ fontSize: 14, fontWeight: 800, color: '#0f172a', margin: '0 0 6px' }}>No complaints found</p>
                ...
                <thead>
                  <tr style={{ background: '#f8fafc', borderBottom: '1.5px solid #f1f5f9' }}>
                    {['ID', 'Customer', 'Type', 'Order', 'Seller', 'Status', 'Date', ''].map(h => (
                      <th key={h} style={{ padding: '12px 16px', fontSize: 10, fontWeight: 800,
        ...
        {!loading && complaints.length > 0 && (
          <p style={{ fontSize: 12, color: '#94a3b8', fontWeight: 600, margin: '12px 0 0', textAlign: 'right' }}>
