...
import { Seller, PaginatedResponse } from '@/types'  // ✅ removed SellerStatus
import { format } from 'date-fns'

type ActionType = 'approve' | 'reject' | 'suspend'

// ✅ Derive status from boolean fields — no row.status needed
  ...
  const columns: Column<Seller>[] = [
    {
      ...
      render: (row) => (
        <div>
          {/* ✅ row.name and row.email directly — sellers are flat User objects */}
    ...
    },
    {
      ...
      header: 'Status',
      render: (row) => {
        // ✅ derive status from booleans — no row.status property
    ...
    },
    {
      ...
      header: 'Actions',
      render: (row) => {
        // ✅ use derived status for button logic
        const s = getSellerStatus(row)
        return (
    ...
    <div className="space-y-4">
      ...
      >
        <p className="text-text-secondary text-sm mb-4">
          {/* ✅ row.name directly — no row.user?.name */}
          {confirmModal?.type === 'approve' &&
            `Approve "${confirmModal.seller.name}" as a seller on the platform?`}
          {confirmModal?.type === 'reject' &&
            `Reject "${confirmModal?.seller.name}"'s application?`}
